...
def listar_centros_custo(apenas_ativos: bool = True, db: Session = Depends(get_db)):
...
def obter_centro_custo(id: int, db: Session = Depends(get_db)):
...
def criar_centro_custo(payload: CentroCustoCreate, db: Session = Depends(get_db)):
...
def atualizar_centro_custo(id: int, payload: CentroCustoCreate, db: Session = Depends(get_db)):
    obj = db.get(DimCentroCusto, id)
    if not obj:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Centro de custo não encontrado")
...
def desativar_centro_custo(id: int, db: Session = Depends(get_db)):
    obj = db.get(DimCentroCusto, id)
    if not obj:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Centro de custo não encontrado")
    obj.ativo = False
